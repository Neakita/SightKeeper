...
public sealed partial class Composition
{
	private void Setup() => DI.Setup(nameof(Composition))
		...
		.Bind<ReadRepository<ImageSet>>().Bind<ObservableRepository<ImageSet>>()
		.Bind<WriteRepository<ImageSet>>().As(Lifetime.Singleton).To<AppDataImageSetsRepository>()
		...
		.Bind<ImageSetDeleter>().To<AppDataImageSetDeleter>()
