...
public sealed class ProfileDataValidator : AbstractValidator<ProfileData>
{
	public ProfileDataValidator(ObjectsLookupper objectsLookupper)
    {
        ...
        RuleFor(data => data.Weights)
            .NotNull();
        RuleForEach(data => data.Tags)
            .Must((data, profileTag) => objectsLookupper.GetDataSet(profileTag.Tag) == objectsLookupper.GetDataSet(objectsLookupper.GetLibrary(data.Weights!)))
            .When(data => data.Weights != null);
        RuleFor(data => data.Tags).NoDuplicates();
