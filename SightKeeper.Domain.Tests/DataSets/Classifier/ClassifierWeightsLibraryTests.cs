...
public sealed class ClassifierWeightsLibraryTests
{
	...
	public void ShouldCreateWeights()
	{
		...
		var weights = dataSet.WeightsLibrary.CreateWeights(DateTime.UtcNow, ModelSize.Nano, new WeightsMetrics(), new Vector2<ushort>(320, 320), null, [tag1, tag2]);
	...
	public void ShouldNotCreateWeightsWithOneTag()
	{
		...
		Assert.ThrowsAny<Exception>(() => dataSet.WeightsLibrary.CreateWeights(DateTime.UtcNow, ModelSize.Nano, new WeightsMetrics(), new Vector2<ushort>(320, 320), null, [tag]));
		dataSet.WeightsLibrary.Weights.Should().BeEmpty();
	}
	...
	public void ShouldNotCreateWeightsWithDuplicateTags()
	{
		...
		var tag1 = dataSet.TagsLibrary.CreateTag("1");
		var tag2 = dataSet.TagsLibrary.CreateTag("2");
		Assert.ThrowsAny<Exception>(() => dataSet.WeightsLibrary.CreateWeights(DateTime.UtcNow, ModelSize.Nano, new WeightsMetrics(), new Vector2<ushort>(320, 320), null, [tag1, tag1, tag2]));
		dataSet.WeightsLibrary.Weights.Should().BeEmpty();
	}
}